...
class TAPBannerUtil {
    public static ImageView getItemImageView(Context context, @DrawableRes int resId, TAPLocalImageSize localImageSize, ImageView.ScaleType scaleType) {
    ...
    /**
     *  Pair 在某些情况下，既需要已键值的方式存储数据列表，还需要在输出的时候保持顺序。HashMap满足前者，ArrayList则满足后者
     * @param <S>
     */
